...
public class Palindrome1One {
    ...
    //Method that describes FIRST way to consider that String is palindrome
    public  String checkFirstCase(String line){
        ...
        for (int i = 0; i <= ((line.length() - 1 ) / 2); i++) {
            if (line.charAt(i) == line.charAt(line.length() - 1 - i) ) {
                //If compared symbols are equals each other than counter will increase to one point
                System.out.println(line.charAt(i) + " " +   line.charAt(line.length() - 1 - i));
                counter++;
            }
        }
        //If counter is equals to a half of quantity of symbols, then string is palindrome
        //Even if number of chars in palindrome sting is odd,
        if (counter  == ((line.length() / 2)) && counter > 0 ){
            return "Palindrome";
        } else {
